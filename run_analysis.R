run_analysis <- function() {
  ...
  activities <- as.data.frame(activity_labels[Y[, 1], 2])
  ...
  #groupColumns <- names(finalData)[1:2]
  #dataColumns <- names(finalData)[-(1:2)]
  #avgData <- ddply(finalData, groupColumns, function(x) colMeans(x[dataColumns]))
  avgData <- finalData %>%
